...
<h2 class="text-xl font-bold mb-6">Kelola Data Ekspor Impor Nasional HS 2 Digit Desember 2025</h2>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-md p-6 flex flex-col justify-between">
        <div>
            ...
            <p class="text-gray-500 text-sm mb-4">
                Input data ekspor impor secara manual berdasarkan Kode HS.
            </p>
        </div>

        <a href="{{ route('admin.data.hs.create') }}"
    ...
    <div class="bg-white rounded-2xl shadow-md p-6">
        <h3 class="text-lg font-semibold mb-2">Import Data HS</h3>
        <p class="text-gray-500 text-sm mb-4">
            Upload file Excel (.xlsx / .xls / .csv)
        </p>

        <form action="{{ route('import.hs') }}" 
              method="POST" 
    ...
    <table class="w-full border-collapse">
        ...
        <tr class="border-b hover:bg-gray-100 {{ session('highlight_id') == $item->id ? 'bg-green-200' : '' }}">
                ...
                <td class="p-3 flex gap-2">
                    <a href="{{ route('admin.data.hs.edit',$item->id) }}"
                       class="bg-yellow-500 text-white px-3 py-1 rounded">Edit</a>

                    <form id="delete-form-{{ $item->id }}"
                    action="{{ route('admin.data.hs.delete',$item->id) }}"
                          method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button"
